...
2. Every Bash/PowerShell command is rewritten (via `updatedInput`) to run
   ...
   (ported verbatim from the sibling pr-review-bot project, except the
   sink directory name -- see `_SINK_DIR`'s own comment for why this one
   line is a deliberate, documented exception to this file's usual
   byte-identical-across-repos rule).
...
# The directory name is the one deliberate byte-for-byte difference from the
# sibling pr-review-bot project's copy of this file (see CLAUDE.md's hook
# parity rule): each repo's sink must be named after its own project so the
# two never collide on a shared machine, and this name is already reserved
# for it in check_exfiltration.py's _PROTECTED list.
_SINK_DIR = Path.home() / ".cache" / "onboarding-wizard-redact"
